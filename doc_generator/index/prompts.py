...
def create_code_file_summary(file_path: str, project_name: str, file_contents: str, content_type: str, file_prompt: str) -> str:
    return f"""
    You are acting as a {content_type} documentation expert for a project called {project_name}.
...
def create_code_questions(file_path: str, project_name: str, file_contents: str, content_type: str, target_audience: str) -> str:
    return f"""
    You are acting as a {content_type} documentation expert for a project called {project_name}.
    Below is the {content_type} from a file located at `{file_path}`. 
    What are 3 questions that a {target_audience} might have about this {content_type}? 
...
def folder_summary_prompt(folder_path: str, project_name: str, files: List[FileSummary], folders: List[FolderSummary], content_type: str, folder_prompt: str) -> str:
    files_summary = "\n".join([f"""
        Name: {file.file_name}
