...
def test_process_file():
    with patch('builtins.open', mock_open(read_data='file contents')):
        with patch('doc_generator.index.create_vector_store.should_ignore', return_value=False):
...
def test_process_file_ignore():
    with patch('doc_generator.index.create_vector_store.should_ignore', return_value=True):
    ...
    with patch('builtins.open', side_effect=Exception('Read error')):
        with patch('doc_generator.index.create_vector_store.should_ignore', return_value=False):
            with patch('builtins.print') as mock_print:
                ...
                assert doc is None
                mock_print.assert_called_with('Error reading file error.py: Read error')
...
def test_process_directory(tmp_path):
    ...
    with patch('doc_generator.index.create_vector_store.should_ignore', side_effect=lambda x, y: x.endswith('.txt')):
...
def test_repo_loader_load():
    with patch('doc_generator.index.create_vector_store.process_directory', return_value=['doc1', 'doc2']) as mock_process_directory:
...
def test_create_vector_store(tmp_path):
    ...
    raw_docs = [Document(page_content='Content 1', metadata={'source': 'file1.py'}),
                Document(page_content='Content 2', metadata={'source': 'file2.py'})]

    # Mock RepoLoader
    with patch.object(RepoLoader, 'load', return_value=raw_docs):
        # Mock text splitter
        with patch.object(RecursiveCharacterTextSplitter, 'split_documents', return_value=raw_docs) as mock_split:
            # Mock HNSWLib and embeddings
            with patch('doc_generator.index.create_vector_store.get_embeddings') as mock_get_embeddings:
                mock_get_embeddings.return_value = MagicMock()
                with patch('doc_generator.index.create_vector_store.HNSWLib') as mock_hnswlib:
                    mock_vector_store = MagicMock()
                    mock_hnswlib.from_documents.return_value = mock_vector_store
